...
**Suggested Branch:** `[YOUR-INITIALS]-issue-25-dashboard-profile-sell`
...
## Checklist
- [ ] **Prerequisite:** Ensure Issues 06, 07, 12, 23 are completed.
- [ ] Create `src/app/dashboard/profile/page.tsx`
- [ ] Create `src/app/sell/page.tsx`

## Files to Create

### File 1 — `src/app/dashboard/profile/page.tsx`
...
export default function DashboardProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      setForm({ avatarUrl: user.avatarUrl || '', bio: user.bio || '', location: user.location || '', instagram: social?.instagram || '', twitter: social?.twitter || '', website: social?.website || '' });
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await updateProfileAction({ avatarUrl: form.avatarUrl, bio: form.bio, location: form.location, socialLinks: { instagram: form.instagram, twitter: form.twitter, website: form.website } });
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-card p-6 md:p-8 max-w-2xl">
        <div className="space-y-5">
          <div className="flex flex-col sm:flex-row items-center gap-6 mb-6">
            <div className="relative w-24 h-24 rounded-full overflow-hidden bg-surface-dark border-4 border-white shadow-sm">
              {form.avatarUrl ? (
            ...
            <div>
              <p className="font-body text-sm text-text font-semibold mb-2">Profile Picture</p>
              <CloudinaryUploadButton onUpload={(url) => setForm({ ...form, avatarUrl: url })}>
                Upload Image
