...
**Suggested Branch:** `[YOUR-INITIALS]-issue-08-productcard-starrating-carousel`


**Labels:** `feature`, `frontend` | **Priority:** 🟡 High | **Depends on:** Issue 07

## Checklist
- [ ] **Prerequisite:** Ensure Issue 07 are completed.
- [ ] Create `src/components/ui/ProductCard.tsx`
- [ ] Create `src/components/ui/StarRating.tsx`
- [ ] Create `src/components/ui/InfiniteCarousel.tsx`
- [ ] Create `src/components/ui/LovedThisWeekCarousel.tsx`

## Files to Create

### File 1 — `src/components/ui/ProductCard.tsx`
...
### File 2 — `src/components/ui/StarRating.tsx`
...
### File 3 — `src/components/ui/InfiniteCarousel.tsx`
...
export default function InfiniteCarousel({
  ...
  return (
    ...
    >
      {/* Gradient fade edges */}
      <div className={`pointer-events-none absolute inset-y-0 left-0 w-16 md:w-24 z-10 bg-linear-to-r ${edgeFadeBg} to-transparent`} />
      <div className={`pointer-events-none absolute inset-y-0 right-0 w-16 md:w-24 z-10 bg-linear-to-l ${edgeFadeBg} to-transparent`} />
      ...
      >
        {items.map((child, i) => (
          <div key={`a-${i}`} className="shrink-0 w-55 sm:w-75">
            {child}
          </div>
        ))}
        {items.map((child, i) => (
          <div key={`b-${i}`} className="shrink-0 w-55 sm:w-75" aria-hidden="true">
            {child}
          </div>
        ))}
      </div>
...
### File 4 — `src/components/ui/LovedThisWeekCarousel.tsx`
